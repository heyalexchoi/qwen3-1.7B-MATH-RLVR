...
2. step
...
Verify: check the final answer with a method that could actually catch an error, and SHOW its arithmetic — never just assert agreement. PREFER a check independent of how you derived the answer: invert the operation (expand a factorization, differentiate an integral, square a root) or plug the answer into the original constraints. If the problem admits no independent check (e.g. evaluating a numeric expression), recompute by a visibly different route and call it a recompute — do NOT label a recompute an "independent check." If the check disagrees with your answer, one of the two is wrong: find and fix the error — never write an equality you did not actually compute. (For "count the ways" / "find the minimum" types where substitution is impossible, do the cheapest real sanity check — recount a small case, check bounds/parity/units — and name it.)
===SOLUTION===
short clean final solution, ending in \\boxed{ANSWER}
